...
export const DirectDirectiveModal: React.FC<Props> = ({
  ...
  return createPortal(
    ...
    >
      <div
        className="bg-white rounded-[2.5rem] w-full max-w-lg h-[80vh] flex flex-col shadow-2xl relative animate-in zoom-in-95 duration-500 overflow-hidden"
      ...
      >
        ...
        <div className="p-10 pb-6 flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-900 tracking-tight">Announcements from your supervisor</h2>
              <p className="text-sm text-slate-500 font-medium">Newest messages appear first.</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 px-10 pb-4 flex flex-col">
          <div className="flex-1 min-h-0 overflow-y-auto scrollbar-hide space-y-4 pr-1">
            {items.length === 0 ? (
              <p className="text-sm font-medium text-slate-500 italic py-8">No directives yet.</p>
            ) : (
              items.map((a, idx) => (
                <div
                  key={a.id}
                ...
                >
                  ...
                  <p className="text-sm font-medium text-slate-700 leading-relaxed italic whitespace-pre-wrap mt-2">"{a.message}"</p>
        ...
        {latestBroadcast && !acknowledgedIds.includes(latestBroadcast.id) ? (
          <div className="p-10 pt-4 flex-shrink-0 border-t border-slate-100">
            <button
