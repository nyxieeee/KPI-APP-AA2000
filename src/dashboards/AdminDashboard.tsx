...
const AdminDashboard: React.FC<Props> = ({
  ...
  const { deptAvgScore, deptPerfRatio, deptQuarterScores } = useMemo(() => {
    ...
    // "Overall so far" should grow as quarters are completed:
    // - after Q1 ends -> overall uses Q1
    // - after Q2 ends -> overall uses Q1+Q2
    // - ...and so on.
    const now = new Date();
    const year = now.getFullYear();
    const quarterEndDates = [
      new Date(year, 2, 31, 23, 59, 59, 999), // Q1 ends March 31
      new Date(year, 5, 30, 23, 59, 59, 999), // Q2 ends June 30
      new Date(year, 8, 30, 23, 59, 59, 999), // Q3 ends Sep 30
      new Date(year, 11, 31, 23, 59, 59, 999), // Q4 ends Dec 31
    ];

    const completedQuarterIndexes = quarterEndDates
      .map((end, idx) => (now.getTime() >= end.getTime() ? idx : -1))
      .filter((idx) => idx !== -1);

    const useIndexes =
      completedQuarterIndexes.length > 0
        ? completedQuarterIndexes
        : // If no quarter is completed yet, fall back to quarters that have data
          quarterCounts
            .map((c, idx) => (c > 0 ? idx : -1))
            .filter((idx) => idx !== -1);

    const overallTotal = useIndexes.reduce((sum, idx) => sum + quarterTotals[idx], 0);
    const overallCount = useIndexes.reduce((sum, idx) => sum + quarterCounts[idx], 0);
    const overallScore =
      overallCount > 0 ? Math.max(0, Math.min(100, overallTotal / overallCount)) : 0;

    return {
      deptAvgScore: score,
      deptPerfRatio: overallScore / 100,
  ...
  // Edit weighted scores → Commit (with maintenance), Load standard confirm, or department Reset. Drafts stay in local state / grading session until Commit.
            ...
            <div className="space-y-3">
              {topPerformers.map((emp, i) => {
                ...
                return (
                  <div
                    key={emp.name}
                    className={`flex flex-wrap items-center gap-4 p-4 rounded-2xl border transition-all ${
                      isTop3
                        ? `${medalBg[i]} shadow-sm`
                  ...
                  >
                    {/* Rank */}
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 font-black text-lg leading-none ${
                    ...
                    <div className="flex-1 min-w-[160px]">
                      <p className="text-sm font-black text-slate-900 truncate leading-tight">{emp.name}</p>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest truncate leading-tight">
                        {emp.dept} • {emp.count} submission{emp.count !== 1 ? 's' : ''}
                      </p>
                    </div>

                    {/* Compare with last month */}
                    {diff != null && (
                      <div className={`flex items-center gap-1 px-2.5 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest shrink-0 whitespace-nowrap leading-none ${
                    ...
                    {/* Score */}
                    <div className={`text-xl sm:text-2xl font-black tabular-nums shrink-0 leading-none ${
        ...
        <div>
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Score movement per employee</p>
            </div>
          </div>
          ...
          {(() => {
            ...
            return (
              <div className="bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden">
                <div className="grid grid-cols-[1fr_110px_110px_130px] gap-0 text-[10px] font-black uppercase tracking-widest text-slate-400 px-5 py-3 border-b border-slate-100">
                  <span>Employee</span>
                ...
                <div className="overflow-x-auto">
                  ...
                  return (
                    <div
                      key={row.name}
                      className="grid grid-cols-[1fr_110px_110px_130px] gap-0 items-center px-5 py-3.5 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors"
                    >
                      <div className="min-w-0">
                        <p className="text-sm font-bold text-slate-800 truncate">{row.name}</p>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest truncate">{row.dept}</p>
                      </div>
                      <div className="text-center text-sm font-black text-slate-400 tabular-nums whitespace-nowrap">
                        {row.last != null ? `${row.last}%` : '—'}
                      </div>
                      <div className={`text-center text-sm font-black tabular-nums whitespace-nowrap ${
                        row.current != null ? 'text-slate-900' : 'text-slate-300'
                      }`}>
                        {row.current != null ? `${row.current}%` : '—'}
                      </div>
                      <div className={`flex items-center justify-center gap-1 px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap ${
        ...
        <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6">
          <div className="flex items-start justify-between gap-6 flex-wrap">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Administrative Dataset</p>
                <h2 className="text-xl font-black text-[#1e293b] uppercase tracking-tight leading-none">ADMIN DATA CONTROL</h2>
                <p className="mt-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Exports, distribution, and purge controls.</p>
              </div>
            </div>
          </div>
        ...
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6 items-stretch">
          <div>
            ...
            <div className="flex flex-col gap-6 h-[75vh] max-h-[75vh]">
              <div className="relative bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-7 flex flex-col flex-[1.4] min-h-0 overflow-hidden">
              <div className="absolute top-6 right-6 w-10 h-10 rounded-2xl bg-slate-50 text-slate-700 flex items-center justify-center border border-slate-100 shadow-sm">
                <Users className="w-5 h-5" />
              </div>
              <div className="pr-14">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Distribution</p>
              ...
              <div className="mt-4 flex items-center justify-end">
                <button
                  ...
                  className={`px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:from-blue-700 hover:to-indigo-700 transition-all shadow-lg shadow-blue-600/20 text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 ${
              ...
              <div className="mt-5 flex-1 min-h-0 overflow-y-hidden space-y-1 pr-0">
                {deptsToShow.map((dept) => {
                  ...
                  return (
                    <div key={dept} className="flex items-center justify-between gap-2 bg-slate-50 border border-slate-100 rounded-2xl px-3 py-1.5">
                      <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest truncate">{dept}</span>
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-xl bg-white border border-slate-200 text-slate-800 hover:bg-slate-50 transition-all shadow-sm text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-1.5 ${
            ...
            {/* Purge employee audits removed from Admin Data Control */}
          </div>
  ...
  const renderRegistry = () => {
    ...
    const renderRow = (userName: string) => {
      ...
      return (
        <div
          key={userName}
          className={`group flex items-center justify-between p-5 rounded-[2rem] transition-all border ${baseBg}`}
        >
          <div className="flex items-center gap-4">
            <div
              className={`w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-xs font-black shadow-sm border-2 ${
                isActive
                ...
                <div className="relative" data-transfer-menu="true">
                  ...
                  {transferringNode === userName && (
                    <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-2xl shadow-2xl border border-slate-100 py-2 z-50 animate-in fade-in slide-in-from-top-2">
                      <p className="px-4 py-2 text-[8px] font-black text-slate-300 uppercase tracking-widest border-b border-slate-50 mb-1">
                        Transfer to Dept
                      </p>
                      {availableDepts
    ...
    return (
      <div className="bg-transparent rounded-none p-0 shadow-none border-0 animate-in fade-in slide-in-from-bottom-2 duration-500 flex flex-col space-y-6">
          ...
          >
            {availableDepts.map(dept => (
              <button
                ...
                className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all duration-300 ease-out transform-gpu ${
                  activeDept === dept
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2.6fr] gap-6 items-stretch bg-slate-50 rounded-[2rem] pt-7 pb-3 px-6 lg:pt-9 lg:pb-4 lg:px-10 shadow-lg border border-slate-200/70 text-slate-900 relative overflow-hidden min-h-[176px] mb-6">
          ...
          <div className="flex flex-col gap-1.5 min-w-0 justify-start pb-2 border-b border-slate-200/70 lg:pb-0 lg:border-b-0">
            ...
            <div className="w-44 self-center -mt-1 mb-1">
              {/* Stacked bar indicator (Active vs Disabled) */}
              <div className="h-4 rounded-full bg-slate-200 border border-slate-200 overflow-hidden shadow-inner">
                <div
                  className="h-full bg-emerald-500 rounded-full transition-none"
                  style={{ width: `${Math.max(0, Math.min(100, animatedActiveRatio * 100))}%` }}
                  aria-label="Active share"
                />
              </div>

              <div className="mt-3 flex items-center justify-between gap-3 text-[9px] font-black uppercase tracking-[0.18em] text-slate-500">
              ...
              <div className="mt-2 text-center">
                <span className="text-[9px] font-black uppercase tracking-[0.18em] text-slate-500">Active nodes</span>
          ...
          <div className="rounded-[1.75rem] bg-white/40 border border-slate-200/70 p-5 lg:p-6 w-full">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
              ...
              <div className="flex flex-col justify-start min-w-0 pt-2 lg:pt-0">
                <div className="flex items-center gap-4 mb-1 min-h-[40px]">
                  ...
                  <div className="min-w-0">
                    ...
                    <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mt-1.5">
                      Overall as quarters complete
                    </h3>
                  </div>
                </div>
                <div className="flex items-center justify-center gap-6 -mt-1.5">
                  <div className="w-44 flex flex-col items-center justify-center flex-shrink-0">
                    <div className="w-full h-4 rounded-full bg-slate-200 border border-slate-200 overflow-hidden shadow-inner">
                      <div
                        className="h-full rounded-full bg-gradient-to-r from-sky-400 via-blue-400 to-emerald-400 transition-none"
                        style={{ width: `${Math.max(0, Math.min(100, animatedDeptPerfRatio * 100))}%` }}
                        aria-label="Department average"
                      />
                    </div>
                ...
                <div className="mt-4 lg:mt-5 grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {['Q1', 'Q2', 'Q3', 'Q4'].map((label, idx) => {
                    const value = deptQuarterScores[idx] || 0;
                    const animatedVal = animatedDeptQuarterScores[idx] ?? 0;
                    const pct = Math.round(value);
                    const animPct = Math.min(100, Math.max(0, animatedVal));
                    return (
                      <div
                        key={label}
                        className="rounded-xl border border-slate-200/70 bg-white/35 px-2.5 py-2 flex flex-col items-center gap-1.5"
                      >
                        ...
                        <div className="w-full h-2 rounded-full bg-slate-200 overflow-hidden">
                          <div
                            className="h-full rounded-full bg-[#3880F0] transition-none"
                            style={{ width: `${animPct}%` }}
              ...
              <div className="flex flex-col justify-start min-w-0 py-2 lg:py-0">
                ...
                <div className="mt-4 max-h-28 lg:mt-5 lg:max-h-32 overflow-y-auto pr-1 space-y-1.5 custom-scrollbar-thin">
                  {departmentSummaries.map(summary => (
                    <div key={summary.dept} className="flex items-center gap-2 py-0.5">
                      <span className="w-20 text-[9px] font-black uppercase tracking-[0.18em] text-slate-500">
                        {summary.dept}
            </span>
                      ...
                      <span className="w-10 text-right text-[9px] font-black text-slate-600 tabular-nums">
        ...
        <div className="space-y-4 rounded-[2rem] border border-slate-200 bg-white/70 shadow-sm p-6">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex flex-col gap-2 flex-1">
                ...
                <div className="relative group w-full max-w-md">
                  ...
                  <input
                    ...
                    className="pl-12 pr-5 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-[11px] font-black text-black tracking-[0.05em] focus:outline-none focus:ring-4 focus:ring-blue-500/15 w-full transition-all focus:bg-white focus:border-blue-200"
              ...
              <div className="flex items-center gap-3 flex-wrap justify-between md:justify-end">
                <div className="flex bg-slate-100/90 p-1.5 rounded-2xl border border-slate-200/60 shadow-inner gap-1">
                  ...
                  className="flex items-center gap-2.5 px-5 py-3 rounded-2xl bg-blue-600 text-white text-[11px] font-black uppercase tracking-[0.2em] shadow-md shadow-blue-600/25 hover:bg-blue-500 hover:shadow-lg hover:shadow-blue-600/30 active:scale-[0.98] transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-white"
                >
          ...
          {isProvisioning && (
            <div className="fixed inset-0 z-[5100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
              <div className="w-full max-w-lg bg-white rounded-[2rem] shadow-2xl border border-slate-100 p-8 space-y-6 animate-in slide-in-from-bottom-4 duration-300">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-black text-slate-900 uppercase tracking-widest">Add new personnel</p>
                    </div>
                  </div>
                  <button
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] font-black text-blue-600/60 uppercase tracking-widest ml-1">Full Name</label>
                <input 
                  type="text"
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  {provisionRoleOpen && (
                    <div
                      className="absolute left-0 right-0 top-full mt-1.5 py-1.5 bg-white border border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 z-50 overflow-hidden"
                      role="listbox"
                    >
                      {(activeDept === 'Admin'
                        ? [UserRole.ADMIN]
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] font-black text-blue-600/60 uppercase tracking-widest ml-1">Department</label>
                <div className="relative">
                  <button
                    type="button"
                    className="w-full pl-4 pr-10 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-sm text-slate-900 cursor-pointer text-left outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-200 transition-all hover:bg-slate-100 hover:border-slate-300 flex items-center"
                    aria-haspopup="listbox"
                ...
                <div className="space-y-1.5 animate-in fade-in slide-in-from-left-2">
                  <label className="text-[9px] font-black text-red-600 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  <button onClick={handleCommitProvision} className="px-8 py-3 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-blue-500/20 active:scale-95 transition-all">Save user changes</button>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="mt-4 flex-1 min-h-0">
            {filteredUsers.length === 0 ? (
              <div className="h-[22rem] min-h-[22rem] flex flex-col items-center justify-center text-center text-slate-400 space-y-2 border border-dashed border-slate-200 rounded-[2rem]">
  ...
  const renderLogs = (heightClass: string = 'min-h-[750px]') => (
    <div className={`flex flex-col ${heightClass} rounded-[2rem] border border-slate-200 bg-slate-50 p-6 md:p-8 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-500`}>
      <div className="flex flex-col gap-6 mb-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-sm font-black text-[#1e293b] uppercase tracking-widest leading-none mb-1">GLOBAL SYSTEM LOGS</h3>
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">REAL-TIME AUDITS</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-5 bg-white/80 rounded-3xl border border-slate-200 shadow-sm">
          ...
          <div className="space-y-1.5">
            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Severity Tier</label>
            <div className="relative">
              <select 
                className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2 text-[10px] font-bold text-slate-700 outline-none appearance-none cursor-pointer focus:border-blue-600"
          ...
          <div className="flex items-end">
            <button 
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all"
            >
      ...
      <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
        {filteredLogs.length === 0 ? (
          <div className="min-h-[360px] flex flex-col items-center justify-center space-y-4 rounded-2xl border border-slate-200 bg-slate-50/70">
            <Filter className="w-16 h-16 text-slate-400" />
            <p className="text-sm font-black uppercase tracking-[0.2em] text-slate-600">No logs match filter criteria</p>
          </div>
        ) : (
          filteredLogs.map((log) => (
            <div key={log.id} className="p-4 bg-white rounded-2xl border border-slate-200 shadow-sm flex gap-4 font-mono text-sm group hover:border-slate-300 hover:shadow transition-all">
              ...
              <div className="space-y-1.5 flex-grow">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-black text-slate-300 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
  ...
  const renderValidation = () => {
    ...
    const recommendedLabel = (s: Transmission['supervisorRecommendation']) =>
      s === 'rejected' ? 'REJECTED (by supervisor)' : 'APPROVED (by supervisor)';
    ...
    return (
      <div className="bg-transparent rounded-none p-0 shadow-none border-0 animate-in fade-in slide-in-from-bottom-2 duration-500 flex flex-col space-y-6">
        <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Admin grade control</p>
        ...
        {filtered.length === 0 ? (
          <div className="min-h-[220px] flex flex-col items-center justify-center space-y-3 rounded-[2rem] border border-slate-200 bg-slate-50/60 p-10 text-center">
            <ClipboardCheck className="w-12 h-12 text-slate-300" />
            <p className="text-sm font-black uppercase tracking-[0.2em] text-slate-600">No grades awaiting Admin Validation</p>
              ...
              {filtered
                ...
                .map((t) => {
                  ...
                  return (
                    <div
                      key={t.id}
                      className="group flex items-center justify-between gap-4 p-5 rounded-[2rem] border border-slate-100 hover:border-blue-200 hover:shadow-sm transition-all"
                    >
                      <div className="min-w-0">
                        <div className="flex items-center gap-3 flex-wrap">
                          <div className="w-12 h-12 rounded-2xl bg-white border border-slate-100 flex items-center justify-center text-xs font-black shadow-sm">
                            {t.userName.charAt(0)}
                          </div>
                          <div className="min-w-0">
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          ...
                          <span className={`px-3 py-1 rounded-full border text-[9px] font-black uppercase tracking-widest ${scoreBg}`}>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <button
                          ...
                          onClick={() => {
                            ...
                            triggerToast('Finalized: Approved', `TX ${t.id} finalized and approved by admin.`);
                          }}
                          className="px-4 py-3 rounded-xl bg-emerald-600 text-white text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-colors"
                        >
                          Finalize: Approve
                        </button>
                        <button
                          ...
                          onClick={() => {
                            ...
                            triggerToast('Finalized: Needs Changes', `TX ${t.id} finalized — needs changes.`);
                          }}
                          className="px-4 py-3 rounded-xl bg-red-600 text-white text-[10px] font-black uppercase tracking-widest hover:bg-red-700 transition-colors"
                        >
                          Finalize: Needs Changes
                        </button>
                      </div>
                    </div>
                  );
    ...
    return (
      ...
      <div className="bg-transparent rounded-none p-0 shadow-none border-0 animate-in fade-in slide-in-from-bottom-2 duration-500 flex flex-col space-y-6">
        ...
        <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Configuration</p>
                <h3 className="text-xl font-black text-[#1e293b] uppercase tracking-tight leading-none">
                  Grading systems
                </h3>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Department coefficients & incentive matrix</p>
              </div>
            </div>
            <div className="w-full sm:w-[22rem] shrink-0 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Access</p>
        ...
        <div className="space-y-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-1">
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] shrink-0">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-700 shadow-sm hover:bg-slate-50 hover:border-slate-300 transition-colors disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-white disabled:hover:border-slate-200"
              >
              ...
              <button
                ...
                className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-[9px] font-black uppercase tracking-widest shadow-sm transition-colors ${
                  standardSnapshotExists
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {gradingDepts.map((dept) => {
              ...
              return (
              <div key={dept} className="group bg-white rounded-[1.75rem] border border-slate-200/90 shadow-lg shadow-slate-200/50 overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-slate-200/60 hover:border-slate-300/80">
                <div className={`px-6 py-5 bg-gradient-to-br ${accentClases} border-b border-slate-100 flex flex-wrap items-center justify-between gap-3`}>
        <div>
                    <h5 className="text-sm font-black text-slate-900 uppercase tracking-[0.2em]">{dept}</h5>
                    <p className="text-[9px] font-bold text-slate-500 uppercase tracking-[0.25em] mt-1">Category weights</p>
            </div>
                  <div className="flex items-center gap-2 flex-wrap justify-end">
                    <span className={`text-[10px] font-black uppercase tracking-widest ${sumValid ? 'text-emerald-600' : 'text-amber-600'}`}>
                      Total: {deptSum}%
                    </span>
                    <button
                      type="button"
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white/80 text-slate-600 text-[9px] font-black uppercase tracking-widest hover:bg-slate-50 transition-colors disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-white/80"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 bg-white text-slate-700 text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-white disabled:hover:border-slate-200"
                    >
        ...
        <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6 md:p-8">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-blue-600/10 border border-blue-200 rounded-xl flex items-center justify-center shadow-sm">
                <TrendingUp className="w-5 h-5 text-blue-600" />
                  ...
                  <div>
                <h4 className="text-sm font-black text-[#1e293b] uppercase tracking-widest">Incentive eligibility matrix</h4>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Applies to all departments & employees</p>
                  </div>
                </div>
            <button
              onClick={handleSaveMatrix}
              className="flex items-center gap-2 px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest bg-emerald-600 text-white hover:bg-emerald-500 shadow-md transition-all active:scale-[0.98]"
            >
          ...
          <div className="space-y-4">
            {incentiveTiers.map((tier, i) => (
              <div
                key={i}
                className="group bg-white rounded-[1.75rem] border border-slate-200/90 shadow-lg shadow-slate-200/50 overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-slate-200/60 hover:border-slate-300/80"
              >
                <div className="px-6 py-4 bg-gradient-to-br from-blue-500/10 to-transparent border-b border-slate-100 flex flex-wrap items-center justify-between gap-3">
                  <div className="min-w-[200px]">
                    <h5 className="text-sm font-black text-slate-900 uppercase tracking-[0.2em]">{tier.status || 'Tier'}</h5>
                    <p className="text-[9px] font-bold text-slate-500 uppercase tracking-[0.25em] mt-1">{tier.outcome || 'Outcome'}</p>
                  </div>
                  <div className="flex items-center gap-2 flex-wrap justify-end">
                    <span className="text-[10px] font-black uppercase tracking-widest text-blue-700 bg-white/80 border border-blue-200/80 px-3 py-1.5 rounded-xl">
                ...
                <div className="p-5 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-1">Tier designation</label>
                <input 
                        type="text"
                        value={tier.status}
                    ...
                    <div>
                      <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-1">Yield outcome</label>
                      <input
                        type="text"
                        value={tier.outcome}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-1">Min score requirement</label>
                      <div className="relative">
                        <input
                          type="number"
                          value={tier.minScore}
                    ...
                    <div>
                      <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-1">Payout yield</label>
                      <div className="relative">
                        <input
                          type="number"
                          value={tier.yield}
      ...
      {loadStandardConfirmOpen &&
        createPortal(
          ...
          >
            <div
              className="w-full max-w-md rounded-2xl border border-slate-200/90 bg-white p-6 shadow-[0_25px_80px_rgba(15,23,42,0.18)]"
              onClick={(e) => e.stopPropagation()}
            >
              <h4 id="load-standard-title" className="text-sm font-black text-slate-900 uppercase tracking-widest">
                Load saved standard?
              </h4>
              ...
              <div className="mt-6 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setLoadStandardConfirmOpen(false)}
                  className="px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-[10px] font-black uppercase tracking-widest text-slate-700 hover:bg-slate-50 shadow-sm transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-600/25 hover:bg-blue-700 transition-colors"
                >
                  Load standard
                </button>
              </div>
        ...
        createPortal(
          ...
          >
            <div
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <h4 id="set-standard-title" className="text-sm font-black text-slate-900 uppercase tracking-widest">
    ...
    <div className="w-full max-w-full xl:max-w-[1600px] 2xl:max-w-[1800px] mx-auto px-4 sm:px-6 md:px-8 flex flex-col md:flex-row gap-6 md:gap-8 pb-6 md:pb-12 min-h-0 flex-1 overflow-auto">
      {dataDeleteCountdownOpen && (
        <div className="fixed inset-0 z-[9500] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-[2rem] border border-slate-200 shadow-2xl overflow-hidden">
            <div className="px-8 py-7 border-b border-slate-100 bg-slate-50/60 flex items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Deletion scheduled</h3>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Final confirmation window</p>
                </div>
              </div>
          <button 
                type="button"
            ...
            <div className="px-8 py-7 space-y-5">
              <div className="text-center">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Countdown</p>
                <div className="mt-3 flex items-center justify-center">
                  <div className="w-28 h-28 rounded-[2rem] bg-red-50 border border-red-100 flex items-center justify-center shadow-sm">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex-1 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 transition-all"
                >
                  Cancel deletion
          </button>
                <div className="flex-1 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest bg-red-50 text-red-700 border border-red-100 text-center tabular-nums">
      ...
      {dataAutoPurgeConfirmOpen && (
        ...
        >
          <div
            className="relative bg-white w-full max-w-2xl rounded-[2.5rem] border border-slate-200 shadow-2xl overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="relative px-10 py-8 border-b border-slate-100 bg-slate-50/50 flex items-start justify-between gap-4">
              ...
              <div>
                  <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Enable End-of-Year Auto Purge</h3>
              ...
              <p className="text-[13px] font-bold text-slate-700 leading-7">
               ...
               <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={() => setDataAutoPurgeConfirmOpen(false)}
                  className="flex-1 py-3.5 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 transition-all shadow-sm"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-3.5 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest bg-blue-600 text-white hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/20"
                >
                  Enable auto purge
                </button>
              </div>
      ...
      {dataPurgeOpen && (
        <div className="fixed inset-0 z-[9000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="relative bg-white w-full max-w-2xl rounded-[2.5rem] border border-slate-200 shadow-2xl overflow-hidden">
            <div className="absolute -top-24 -right-24 h-56 w-56 rounded-full bg-blue-500/10 blur-3xl pointer-events-none" aria-hidden />
            <div className="absolute -bottom-24 -left-24 h-56 w-56 rounded-full bg-cyan-400/10 blur-3xl pointer-events-none" aria-hidden />
            ...
            <div className="relative px-10 py-8 border-b border-slate-100 bg-slate-50/50 flex items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Delete employee audits</h3>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Admin confirmation required</p>
                </div>
               </div>
               <button 
                type="button"
            ...
            <div className="relative px-10 py-8 space-y-6">
              ...
              {dataBackupRequired && (
                <div className="p-4 rounded-[1.5rem] bg-slate-50 border border-slate-100">
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Backup required</p>
                  ...
                  <button
                    ...
                    disabled={dataZipBusy}
                    onClick={buildEmployeeAuditsZip}
                    className={`mt-3 w-full px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${
                      dataZipBusy
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-4 py-3.5 bg-slate-100 text-slate-900 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-red-500/10 outline-none font-bold text-sm transition-all"
                  placeholder="••••••••"
                />
                {dataPurgeErr && (
                  <div className="p-3 rounded-2xl bg-red-50 text-red-700 border border-red-100 text-[10px] font-black uppercase tracking-widest">
                    {dataPurgeErr}
                    </div>
                )}
                  </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-3.5 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest bg-red-600 text-white hover:bg-red-700 transition-all shadow-lg shadow-red-600/20"
                >
                  Confirm delete
                </button>
                      </div>
        ...
        return (
          <>
            ...
            <div className="fixed inset-0 z-[5001] flex items-center justify-center p-4 pointer-events-none" aria-hidden="true">
              <div className="bg-white rounded-[1.75rem] w-full max-w-5xl max-h-[90vh] flex flex-col shadow-xl shadow-slate-900/10 border border-slate-200/90 overflow-hidden animate-in zoom-in-95 duration-200 pointer-events-auto" onClick={(e) => e.stopPropagation()}>
              <div className="px-6 py-5 border-b border-slate-100/80 flex items-center justify-between gap-4 flex-wrap bg-slate-50/60 shrink-0 overflow-visible">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center shadow-md shadow-blue-600/25">
                    <Scale className="w-5 h-5 text-white" />
                  </div>
                  <div>
                    <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Edit weighted scores</h3>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-0.5">{dept} · Name, icon, weight & grading content</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  {gradingExitConfirmOpen && createPortal(
                    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-slate-900/20" onClick={() => setGradingExitConfirmOpen(false)}>
                      <div className="w-full max-w-sm rounded-xl border border-slate-200 bg-white p-5 shadow-xl shadow-slate-900/10" onClick={(e) => e.stopPropagation()}>
              ...
              {gradingCriterionEditor && (
                <>
                  ...
                  <div className="fixed inset-0 z-[5201] flex items-center justify-center p-4">
                    <div
                      className="bg-gradient-to-b from-white to-slate-50/70 rounded-[2rem] w-full max-w-5xl border border-slate-200/70 shadow-[0_30px_120px_rgba(2,6,23,0.22)] p-6 relative overflow-hidden flex flex-col h-[600px] max-h-[85vh]"
                      onClick={(e) => e.stopPropagation()}
                    >
                      <div className="flex items-center justify-between gap-3 mb-4 p-3 rounded-2xl bg-slate-50/70 border border-slate-200/60">
                        <div className="flex items-center gap-2">
                          <Edit2 className="w-5 h-5 text-blue-600" />
                          <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Edit grading criterion</h3>
                        </div>
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            className="px-5 py-3 rounded-xl bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest shadow-lg shadow-slate-900/25 hover:bg-slate-800 hover:shadow-xl hover:shadow-slate-900/30 active:scale-[0.98] transition-all duration-200"
                            onClick={() => {
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-5 flex-1 min-h-0 overflow-y-auto pb-4">
                        <div className="flex flex-col space-y-3 min-h-0">
                          <div className="p-4 rounded-2xl border border-slate-200/60 bg-white/80 shadow-sm">
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Criterion definition</p>
                            <div className="space-y-3">
                              <div className="space-y-1.5">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Definition</label>
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1 leading-snug">
                                ...
                                <div className="w-full max-w-full">
                                  <textarea
                                    ...
                                    className="box-border w-full h-40 max-h-40 min-h-40 resize-none overflow-y-auto bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-[13px] font-black text-slate-900 leading-snug outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-600 transition-all"
                          ...
                          <div className="p-4 rounded-2xl bg-white/80 border border-slate-200/60 shadow-sm">
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Element palette</p>
                            <div className="space-y-2">
                              {(() => {
                                ...
                                return (
                                  <>
                                    <button
                                      ...
                                      className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-blue-600 text-white text-[11px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-blue-600"
                                    >
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center justify-between gap-3">
                            <div>
                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Panel rule canvas</p>
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="bg-white/70 rounded-[2rem] border border-slate-200/70 p-4 md:p-5 space-y-4 shadow-sm backdrop-blur">
                                <div className="flex items-start justify-between gap-4">
                                  <div className="flex min-w-0 flex-1 items-center gap-3">
                                    <div className="w-12 h-12 rounded-2xl bg-white border border-slate-100 flex items-center justify-center shrink-0 relative">
                                      {logoEntry ? (
                      <div className="relative">
                                          <button
                                            type="button"
                                            ...
                                            className="w-12 h-12 rounded-2xl bg-blue-500/10 border border-slate-200 flex items-center justify-center shadow-sm hover:bg-blue-500/20 hover:border-blue-300 transition-colors"
                                    ...
                                    <div className="flex-1 min-w-[min(100%,24.5rem)]">
                                      <input
                                        ...
                                        className="w-full min-w-0 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-2.5 text-[13px] font-black text-slate-900 outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-600 transition-all"
  ...
  return (
                  ...
                  {gradingCriterionExitConfirmOpen &&
                    createPortal(
                      ...
                      >
                        <div
                          className="w-full max-w-sm rounded-xl border border-slate-200 bg-white p-5 shadow-xl shadow-slate-900/10"
                          onClick={(e) => e.stopPropagation()}
                        >
                              ...
                              <>
                                <div className="flex items-start justify-between gap-3 mb-3">
                                  <div>
                                    ...
                                    <p className="text-[11px] font-black text-slate-900 uppercase tracking-widest">Scoring checkpoints</p>
                                    {mode === 'checkbox' && (
                                ...
                                {multiTextboxGrading ? (
                                  <div className="space-y-8 mb-4">
                                    {Array.from({ length: textboxCountForPopup }, (_, ord) => {
                                      ...
                                      return (
                                        <div key={ord} className="rounded-2xl border border-slate-100 bg-slate-50/40 p-4 space-y-3">
                                          <p className="text-[10px] font-black text-slate-700 uppercase tracking-widest">
                                            {textboxSectionTitles[ord] ?? `Textbox ${ord + 1}`}
                                          </p>
                                          <div className="hidden sm:grid gap-2 text-[9px] font-black text-slate-400 uppercase tracking-widest px-1 mb-1 sm:grid-cols-12">
                                            ...
                                            {cps.map((cp, rowIdx) => (
                                              ...
                                              >
                                                <div className="space-y-1 sm:col-span-2">
                                                  <label className="sm:hidden text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                                    Min value
                                                  </label>
                                                  <input
                                                    type="number"
                                          ...
                                          <button
                                            ...
                                            className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 rounded-xl border-2 border-dashed border-blue-200 text-blue-700 text-[10px] font-black uppercase tracking-widest hover:bg-blue-50 transition-colors"
                                          >
                                            <Plus className="w-4 h-4" />
                                            Add checkpoint
                                          </button>
                                  ...
                                  <>
                                    <div
                                      className={`hidden sm:grid gap-2 text-[9px] font-black text-slate-400 uppercase tracking-widest px-1 mb-1 ${
                                    ...
                                    <button
                                      ...
                                      className="mb-4 w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 rounded-xl border-2 border-dashed border-blue-200 text-blue-700 text-[10px] font-black uppercase tracking-widest hover:bg-blue-50 transition-colors"
                                    >
                                      <Plus className="w-4 h-4" />
                                      Add checkpoint
                                    </button>
                                ...
                                <div className="flex justify-end gap-2">
                                  <button
                                    ...
                                    className="px-5 py-3 rounded-xl bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest shadow-lg shadow-slate-900/25 hover:bg-slate-800 hover:shadow-xl hover:shadow-slate-900/30 transition-all"
                                  >
                                    Commit changes
                                  </button>
                                </div>
              ...
              <div className="p-6 overflow-y-auto flex-1 min-h-0">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Edit category name, icon, weight (%), and grading content criteria.</p>
                <div className="space-y-4">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">KPI categories</p>
                    <button
                      type="button"
                      onClick={() => handleAddCategory(dept)}
                      className="px-4 py-2 rounded-xl bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest shadow-sm hover:bg-blue-700 active:scale-[0.99] transition-all flex items-center gap-2"
                      ...
                      >
                        <div className="p-4 space-y-3">
                            ...
                            <div className="flex items-center justify-end gap-2 min-h-[20px] min-w-0 px-2 py-1">
                              <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest shrink-0">Weighted impact</span>
                            ...
                            <div className="flex items-center justify-end gap-2 shrink-0">
                              <button
                                ...
                                className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-slate-200 bg-white text-slate-600 text-[9px] font-black uppercase tracking-widest hover:bg-slate-50 shadow-sm transition-colors shrink-0"
                              ...
                              <button
                                ...
                                className={`flex items-center gap-1.5 px-3 py-2 rounded-lg border text-[9px] font-black uppercase tracking-widest shadow-sm transition-colors shrink-0 ${
                                  categories.length <= 1
                          ...
                          <div>
                            <button type="button" onClick={() => setGradingContentExpanded(isContentExpanded ? null : idx)} className="flex items-center gap-2 text-[10px] font-black text-slate-600 uppercase tracking-widest hover:text-blue-600 transition-colors">
                              ...
                              <div className="mt-3 pl-4 border-l-2 border-slate-200 space-y-2">
                                {content.map((item, itemIdx) => (
                                  ...
                                  >
                                    ...
                                    >
                                      <span className="text-[11px] font-bold text-slate-800 uppercase tracking-widest truncate">
                                ...
                                <button type="button" onClick={() => handleAddContentItem(dept, idx)} className="flex items-center gap-1.5 text-[10px] font-black text-blue-600 uppercase tracking-widest hover:text-blue-700">
      ...
      {editingNode && (
        <div className="fixed inset-0 z-[5000] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] w-full max-w-sm p-10 shadow-2xl border border-slate-100 space-y-8 relative overflow-hidden">
            ...
            <div className="flex justify-between items-center relative">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest leading-none mb-1">Manage user</h3>
            ...
            <div className="space-y-6 relative">
              <div className="space-y-1.5">
                ...
                <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-sm font-black text-slate-900 outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-600 transition-all" value={editingNode.name} onChange={(e) => setEditingNode({...editingNode, name: e.target.value})} />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Access Designation</label>
                <div className="relative">
                  <select 
                    className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-sm font-black text-slate-900 outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-600 appearance-none cursor-pointer disabled:opacity-50" 
            ...
            <div className="flex flex-col gap-3">
              <button onClick={handleSaveEdit} className="w-full bg-slate-900 text-white py-4 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-slate-800 transition-all flex items-center justify-center gap-3"><Save className="w-4 h-4" />Commit Changes</button>
              <button
                onClick={() => editingNode && setUnenrollConfirmName(editingNode.originalName)}
                className="w-full bg-red-600 text-white py-4 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-red-700 transition-all flex items-center justify-center gap-3"
              >
      ...
      {unenrollConfirmName && (
        <div className="fixed inset-0 z-[5010] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[2.5rem] w-full max-w-sm p-8 shadow-2xl border border-slate-100 space-y-6 relative">
            <div className="text-center space-y-2">
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">This will permanently remove the user and its data.</p>
            </div>
            <div className="flex gap-3">
              <button
                onClick={() => setUnenrollConfirmName(null)}
                className="flex-1 py-3.5 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] border-2 border-slate-200 text-slate-600 hover:bg-slate-50 transition-all"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 py-3.5 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] bg-red-600 text-white hover:bg-red-700 transition-all flex items-center justify-center gap-2"
              >
      ...
      {showExportSuccess && (
        <div className="fixed top-24 right-8 z-[9999] animate-in slide-in-from-right-full fade-in duration-500">
          <div className="bg-[#0b1222] text-white px-6 py-4 rounded-[1.5rem] shadow-2xl border border-emerald-500/30 flex items-center gap-4">
            ...
            <div>
              <p className="text-[11px] font-black uppercase tracking-widest mb-1">{toastMessage.title}</p>
      ...
      <div className="flex-grow flex flex-col min-h-0">
        ...
        <div className="mb-8 flex flex-col gap-6 lg:hidden">
          <div>
            ...
            <p className="mt-3 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-slate-100 to-blue-50 border border-slate-200/80 shadow-sm">
              <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Signed in as</span>
        ...
        <div className="hidden lg:block">
          <aside
            className={`fixed left-0 ${APP_NAV_SIDENAV_TOP} z-[60] ${APP_NAV_SIDENAV_HEIGHT} border-r border-slate-200 bg-white text-slate-900 shadow-xl ${
            ...
            <div className="h-full flex flex-col p-4 min-h-0">
              ...
              <div className="mt-auto pt-4 border-t border-slate-100 space-y-2 shrink-0">
                ...
                >
                  ...
                  {!adminNavCollapsed && (
                    <span className="text-[11px] font-black uppercase tracking-widest">Collapse</span>
                  )}
                </button>
                {!adminNavCollapsed && (
