                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{emp.dept} • {emp.count} submission{emp.count !== 1 ? 's' : ''}</p>
                    </div>

                    {/* Compare with last month */}
                    {diff != null && (
                      <div className={`flex items-center gap-1 px-2.5 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest shrink-0 ${
          ...
          {(() => {
            ...
            return (
              <div className="bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden">
                <div className="grid grid-cols-[1fr_auto_auto_auto] gap-0 text-[10px] font-black uppercase tracking-widest text-slate-400 px-5 py-3 border-b border-slate-100">
                  <span>Employee</span>
                ...
                {rows.map(row => {
                  ...
                  return (
                    <div key={row.name} className="grid grid-cols-[1fr_auto_auto_auto] gap-0 items-center px-5 py-3.5 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors">
                      <div>
                        <p className="text-sm font-bold text-slate-800">{row.name}</p>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{row.dept}</p>
                      </div>
                      ...
                      <div className={`flex items-center justify-center gap-1 px-3 py-1 mx-2 rounded-xl text-[10px] font-black uppercase tracking-widest ${
      ...
      <div className="w-full animate-in fade-in slide-in-from-bottom-2 duration-500">
        ...
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6 items-stretch">
          <div>
            ...
            <div className="relative bg-white rounded-[2.5rem] border border-red-100 shadow-sm p-7 flex flex-col flex-[1.0] min-h-0 overflow-hidden">
              <div className="absolute top-6 right-6 w-10 h-10 rounded-2xl bg-red-50 text-red-700 flex items-center justify-center border border-red-100 shadow-sm">
                <Trash2 className="w-5 h-5" />
              </div>
              <div className="pr-14">
                <p className="text-[10px] font-black text-red-500 uppercase tracking-[0.3em]">Danger zone</p>
                <h3 className="mt-2 text-lg font-black text-slate-900 uppercase tracking-tight">Purge employee audits</h3>
              </div>

              <div className="mt-5 bg-red-50/60 border border-red-100 rounded-2xl p-4 flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <p className="text-[10px] font-black text-red-700 uppercase tracking-widest">Year-end auto purge</p>
                  <p className="mt-1 text-[11px] font-bold text-slate-600">
                    {dataAutoPurgeEnabled ? (
                      <>Auto-purge in <span className="font-black text-slate-900 tabular-nums">{dataAutoPurgeCountdown}</span></>
                    ) : (
                      <>Automation is off</>
                    )}
                  </p>
                ...
                <button
                  type="button"
                  onClick={() => {
                    // When enabling, require explicit confirmation.
                    if (!dataAutoPurgeEnabled) {
                      setDataAutoPurgeConfirmOpen(true);
                      return;
                    }
                    setDataAutoPurgeEnabled(false);
                  }}
                  className={`shrink-0 w-24 inline-flex items-center justify-center gap-2 px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border ${
                    dataAutoPurgeEnabled
                      ? 'bg-red-600 text-white border-red-600 shadow-lg shadow-red-600/15'
                      : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50'
                  }`}
                  title="Toggle year-end automation"
                >
                  {dataAutoPurgeEnabled ? <ToggleRight className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />}
                  {dataAutoPurgeEnabled ? 'On' : 'Off'}
                </button>
              </div>
              ...
              <button
                type="button"
                onClick={() => {
                  setDataDeleteHoverTooltipOpen(false);
                  setDataDeleteHoverTooltipRect(null);
                  setDataBackupRequired(true);
                  setDataBackupDone(false);
                  setDataPurgeOpen(true);
                  setDataPurgePwd('');
                ...
                ref={dataDeleteHoverButtonRef}
                onMouseEnter={(e) => {
                  const rect = (e.currentTarget as HTMLButtonElement).getBoundingClientRect();
                  setDataDeleteHoverTooltipRect({ left: rect.left + rect.width / 2, top: rect.top });
                  setDataDeleteHoverTooltipOpen(true);
                }}
                onMouseLeave={() => {
                  setDataDeleteHoverTooltipOpen(false);
                  setDataDeleteHoverTooltipRect(null);
                }}
                className="mt-5 w-full px-5 py-3 rounded-2xl bg-red-600 text-white hover:bg-red-700 transition-all shadow-lg shadow-red-600/20 text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2"
              >
                <Trash2 className="w-4 h-4" />
                Delete employee audits
              </button>

              {dataDeleteHoverTooltipOpen && dataDeleteHoverTooltipRect && createPortal(
                <div
                  className="fixed px-4 py-3 rounded-xl bg-slate-900 text-white text-[11px] font-bold shadow-2xl border border-slate-700/60 whitespace-normal max-w-[320px] z-[99999]"
                  style={{
                    left: dataDeleteHoverTooltipRect.left,
                    top: dataDeleteHoverTooltipRect.top - 8,
                    transform: 'translate(-50%, -100%)',
                  }}
                >
                  <div>
                    This permanently removes all employee audits (pending + history, including validated/rejected).
                    This can’t be undone.
                  </div>
                  <span className="absolute left-1/2 -translate-x-1/2 top-full w-0 h-0 border-[7px] border-transparent border-t-slate-900" />
                </div>,
                document.body
              )}
            </div>
