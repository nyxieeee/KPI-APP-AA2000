...
const MarketingSupervisorDashboard: React.FC<Props> = ({
  ...
  const handleAction = (type: 'APPROVE' | 'REJECT' | 'OVERRIDE') => {
    ...
    if (type === 'REJECT') {
      onAddAuditEntry('KPI_REJECTED', `Supervisor recommended: Needs Changes on TX ${selectedItem.id}.`, 'WARN');
      onSupervisorGrade(
        selectedItem.id,
        { ...(overrides || {}), ratings, supervisorComment: reason },
        'rejected'
      );
      setFeedbackMsg('Recommendation submitted: Needs Changes — awaiting admin finalization.');
    } else {
      onAddAuditEntry(
        'KPI_APPROVED',
        `Supervisor recommended: Approve on TX ${selectedItem.id}. Score: ${calculatedScore.final}%. Awaiting Admin Validation.`,
        'OK'
      );
      onSupervisorGrade(
        selectedItem.id,
      ...
      setFeedbackMsg('Recommendation submitted: Approve — awaiting admin finalization.');
    }
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 mt-1">
                      <p className="text-[9px] font-bold uppercase tracking-widest text-slate-400">TX ID: {item.id} • {item.jobId || '—'}</p>
      ...
      <div className="w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden pb-12">
          ...
          <div className="p-10 space-y-12">
              ...
              <div className="lg:col-span-5 flex flex-col gap-6 min-h-0">
                {!isReadOnly ? (
                  <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm h-full flex flex-col min-h-0">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex-shrink-0">Supervisor Justification</label>
                    <textarea
                  ...
                  <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm space-y-6 h-full flex flex-col justify-center">
                    <div className="space-y-4 text-center">
                      ...
                      <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">Record Finalized</h4>
                    ...
                    <button onClick={() => setCurrentPage('queue')} className="w-full py-5 bg-slate-900 text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] transition-all mt-6">Return to Registry</button>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3"><div className="w-8 h-8 bg-slate-900 rounded-lg flex items-center justify-center"><ClipboardCheck className="w-4 h-4 text-white" /></div><p className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Supervisor Justification</p></div>
              <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3"><div className="w-8 h-8 bg-slate-900 rounded-lg flex items-center justify-center"><Paperclip className="w-4 h-4 text-white" /></div><p className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Global Evidence Registry</p></div>
