...
const MarketingSupervisorDashboard: React.FC<Props> = ({
  ...
  const handleAction = (type: 'APPROVE' | 'REJECT' | 'OVERRIDE') => {
    ...
    if (type === 'REJECT') {
      if (!reason) {
        setJustificationPopupMessage('Please enter your reason for rejecting before sending it to the admin reviewer.');
        return;
      }
      ...
      if (gradingChanged && !reason) {
        setJustificationPopupMessage('You updated the score rules. Please enter your reason before sending your recommendation to the admin reviewer.');
        return;
      }
    } else if (type === 'OVERRIDE' && !reason) {
      setJustificationPopupMessage('Please provide a reason for the manual score update in the field above.');
      return;
    }
    ...
    if (type === 'REJECT') {
      onAddAuditEntry('KPI_REJECTED', `Supervisor recommendation: Needs changes for submission ${selectedItem.id}.`, 'WARN');
      onSupervisorGrade(
        selectedItem.id,
        { ...(overrides || {}), ratings, supervisorComment: reason },
        'rejected'
      );
      setFeedbackMsg('Recommendation sent: needs changes. Waiting for admin approval.');
    } else {
      onAddAuditEntry(
        'KPI_APPROVED',
        `Supervisor recommendation: Approve for submission ${selectedItem.id}. Score: ${calculatedScore.final}%. Waiting for admin approval.`,
        'OK'
      );
      onSupervisorGrade(
        selectedItem.id,
      ...
      setFeedbackMsg('Recommendation sent: approved. Waiting for admin approval.');
    }
  ...
  const renderDashboard = () => (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="bg-slate-100 rounded-[2.5rem] p-8 border border-slate-200 shadow-lg h-[22rem] min-h-[22rem] flex flex-col">
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-4 shrink-0">
              ...
              <div className="flex flex-col items-end">
                ...
                <div className="flex w-full max-w-[22rem] bg-slate-100/90 p-2 rounded-2xl gap-2 shadow-inner border border-slate-200/60">
                  {(['Q1', 'Q2', 'Q3', 'Q4'] as const).map(q => (
                  <button
                    key={q}
                    type="button"
                    onClick={() => setLeaderboardQuarter(q)}
                    className={`flex-1 min-w-[4.5rem] py-2.5 px-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all duration-200 ${
            ...
            <div className="bg-white rounded-[2rem] border border-slate-200/80 shadow-sm p-4 md:p-5 flex-1 min-h-0 overflow-y-auto">
              {departmentLeaderboard.length === 0 ? (
                ...
                <div className="space-y-2">
                  {departmentLeaderboard.map((entry, index) => (
                    <div
                      key={entry.name}
                      className="flex items-center gap-3 rounded-2xl px-3 py-2.5 hover:bg-slate-50 transition-colors"
                    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        <div className="lg:col-span-5 flex flex-col">
          <div className="bg-slate-100 rounded-[2.5rem] p-8 border border-slate-200 shadow-lg overflow-hidden h-[22rem] min-h-[22rem] flex flex-col">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6 shrink-0">
              <div>
                <p className="text-[10px] font-black tracking-[0.3em] text-slate-400 uppercase">Message to your team</p>
                <h2 className="mt-1 text-xl font-black text-slate-900 tracking-tight">New announcement</h2>
              </div>
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/80 border border-slate-200 text-[9px] font-black text-slate-500 uppercase tracking-widest shadow-sm">This department only</span>
            </div>
            <div className="flex-1 min-h-0 flex flex-col gap-4 bg-white rounded-[2rem] p-6 border border-slate-200/80 shadow-sm overflow-hidden">
              <textarea
                value={announcementMsg}
                onChange={e => setAnnouncementMsg(e.target.value)}
                placeholder="Write a message for your team…"
                className="w-full flex-1 min-h-[120px] bg-slate-50/80 border border-slate-100 rounded-xl px-5 py-4 text-sm font-medium text-slate-700 placeholder:text-slate-400 outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-200 transition-all resize-none"
              />
              <button
                onClick={handleDispatchAnnouncement}
                disabled={!announcementMsg.trim()}
                className="w-full flex items-center justify-center gap-2.5 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-lg shadow-blue-200/50 shrink-0"
              >
        ...
        <div className="lg:col-span-7 flex flex-col">
          <div className="bg-slate-100 rounded-[2.5rem] p-8 border border-slate-200 shadow-lg overflow-hidden h-[22rem] min-h-[22rem] flex flex-col">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6 shrink-0">
              <div>
                <p className="text-[10px] font-black tracking-[0.3em] text-slate-400 uppercase">This quarter</p>
                <h2 className="mt-1 text-xl font-black text-slate-900 tracking-tight">Announcements sent</h2>
              </div>
              {activeAnnouncements.length > 0 && (
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-50 border border-blue-100 text-[9px] font-black text-blue-700 uppercase tracking-widest">
                  <span className="inline-block w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />
                  {activeAnnouncements.length} active
                </span>
              )}
            </div>
            <div className="flex-1 min-h-0 flex flex-col bg-white rounded-[2rem] border border-slate-200/80 shadow-sm overflow-hidden">
              {activeAnnouncements.length === 0 ? (
                <div className="flex-1 min-h-0 flex flex-col items-center justify-center p-6 text-center shrink-0">
                  ...
                  <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">No announcements yet</p>
                ...
                <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-4 pr-2 custom-scrollbar">
                  {activeAnnouncements.map(a => (
                    <div
                      key={a.id}
                      className="relative group p-5 rounded-2xl bg-slate-50/80 border border-slate-100 hover:bg-blue-50/60 hover:border-blue-100 hover:shadow-sm transition-all"
                    >
                      <button
  ...
  const renderQueue = () => {
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-slate-100 rounded-[2.5rem] p-8 border border-slate-200 shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <button
            ...
            className={`w-full p-8 rounded-[2rem] border flex flex-col items-center text-left transition-transform duration-200 ease-out cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 focus:ring-offset-slate-100 hover:scale-[1.02] hover:-translate-y-0.5 hover:shadow-lg ${queueTab === 'pending' ? 'bg-blue-50 border-blue-300 shadow-md scale-[1.06] -translate-y-1' : 'bg-white border-slate-200 hover:bg-slate-50'}`}
            aria-label="Show pending audits in registry"
            aria-pressed={queueTab === 'pending'}
          >
            <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${queueTab === 'pending' ? 'text-blue-700' : 'text-slate-400'}`}>Audits Submitted (Pending)</p>
            <p className={`text-[9px] font-bold uppercase tracking-widest mb-4 ${queueTab === 'pending' ? 'text-blue-600/90' : 'text-slate-500'}`}>Marketing Department</p>
            <div className="relative flex items-center justify-center w-40 h-40">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className={`text-[9px] font-bold uppercase tracking-widest mt-1 ${queueTab === 'pending' ? 'text-blue-600' : 'text-slate-400'}`}>submitted</span>
              </div>
            </div>
          </button>
          <button
            ...
            className={`w-full p-8 rounded-[2rem] border flex flex-col items-center text-left transition-transform duration-200 ease-out cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-offset-2 focus:ring-offset-slate-100 hover:scale-[1.02] hover:-translate-y-0.5 hover:shadow-lg ${queueTab === 'history' ? 'bg-emerald-50 border-emerald-300 shadow-md scale-[1.06] -translate-y-1' : 'bg-white border-slate-200 hover:bg-slate-50'}`}
            aria-label="Show validated audits in registry"
            aria-pressed={queueTab === 'history'}
          >
            <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${queueTab === 'history' ? 'text-emerald-700' : 'text-slate-400'}`}>Validated in Department</p>
            <p className={`text-[9px] font-bold uppercase tracking-widest mb-4 ${queueTab === 'history' ? 'text-emerald-600/90' : 'text-slate-500'}`}>Marketing Department</p>
            <div className="relative flex items-center justify-center w-40 h-40">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="42" fill="none" stroke="rgb(226 232 240)" strokeWidth="10" />
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className={`text-[9px] font-bold uppercase tracking-widest mt-1 ${queueTab === 'history' ? 'text-emerald-600' : 'text-slate-400'}`}>validated</span>
              </div>
            </div>
          </button>
          <button
            ...
            className={`w-full p-8 rounded-[2rem] border flex flex-col items-center text-left transition-transform duration-200 ease-out cursor-pointer focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-offset-2 focus:ring-offset-slate-100 hover:scale-[1.02] hover:-translate-y-0.5 hover:shadow-lg ${queueTab === 'rejected' ? 'bg-red-50 border-red-300 shadow-md scale-[1.06] -translate-y-1' : 'bg-white border-slate-200 hover:bg-slate-50'}`}
            aria-label="Show rejected audits in registry"
            aria-pressed={queueTab === 'rejected'}
          >
            <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${queueTab === 'rejected' ? 'text-red-700' : 'text-slate-400'}`}>Rejected Entries</p>
            <p className={`text-[9px] font-bold uppercase tracking-widest mb-4 ${queueTab === 'rejected' ? 'text-red-600/90' : 'text-slate-500'}`}>Marketing Department</p>
            <div className="relative flex items-center justify-center w-40 h-40">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="42" fill="none" stroke="rgb(226 232 240)" strokeWidth="10" />
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className={`text-[9px] font-bold uppercase tracking-widest mt-1 ${queueTab === 'rejected' ? 'text-red-600' : 'text-slate-400'}`}>rejected</span>
              </div>
            </div>
          </button>
        </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-[2rem] border border-slate-100">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Marketing Registry</h3>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
              {queueTab === 'pending' ? 'Pending Marketing Review' : queueTab === 'history' ? 'Validated Marketing Records' : 'Rejected Marketing Records'}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-4 md:gap-6">
          <div className="relative group w-full md:w-auto">
            ...
            <input
              ...
              className="pl-12 pr-5 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-[11px] font-black text-black tracking-[0.05em] focus:outline-none focus:ring-4 focus:ring-blue-500/15 w-full md:w-80 lg:w-96 transition-all focus:bg-white focus:border-blue-200"
              value={searchTerm}
      ...
      <div
        className="registry-list-scroll h-[19rem] min-h-[19rem] rounded-[2rem] border border-slate-200 bg-slate-50/50 shadow-sm"
        onWheelCapture={(e) => {
        ...
        {filteredQueue.length === 0 ? (
          <div className="py-20 text-center bg-white rounded-[2rem] border border-slate-50 border-dashed">
            ...
            <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">
              {searchTerm.trim() ? 'No records match your search' : 'No Marketing records in this tab'}
            </p>
          </div>
        ) : (
          filteredQueue.map(item => {
            ...
            return (
              <div key={item.id} className="group flex items-center justify-between p-6 rounded-[2rem] border bg-white border-slate-50 hover:shadow-md transition-all">
                <div className="flex items-center gap-6">
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center font-black bg-slate-100 text-slate-600 group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 mt-1">
                      <p className="text-[9px] font-bold uppercase tracking-widest text-slate-400">Submission ID: {item.id} • {item.jobId || '—'}</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {(() => {
                    ...
                    return (
                      <div className={`w-[6.5rem] min-w-[6.5rem] py-2 rounded-xl flex flex-col items-center justify-center border shrink-0 ${kpiBg}`}>
                        <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Initial Score</p>
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all bg-slate-900 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                    aria-label={queueTab !== 'pending' ? `View record ${item.id}` : `Grade submission ${item.id}`}
                  >
                    {queueTab !== 'pending' ? 'View Record' : 'Grade'} <Eye className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
            );
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Marketing Unit Matrix</h3>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Department roster and roles</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="px-4 py-2 bg-blue-50 text-blue-600 rounded-xl border border-blue-100">
            <span className="text-[9px] font-black uppercase tracking-widest">{sortedTeam.length} team members</span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        {sortedTeam.length === 0 ? (
          <div className="py-20 text-center bg-white rounded-[2rem] border border-slate-50 border-dashed mx-1">
            ...
            <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">No team members in this department</p>
          </div>
        ) : (
          sortedTeam.map((member, idx) => (
            <div key={idx} className={`group flex items-center justify-between p-6 rounded-[2rem] border transition-all ${member.isSupervisor ? 'bg-blue-50 border-blue-200 shadow-sm shadow-blue-50' : 'bg-white border-slate-50 hover:shadow-md hover:-translate-y-0.5'}`}>
              <div className="flex items-center gap-6">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black transition-colors shrink-0 ${member.isSupervisor ? 'bg-blue-600 text-white ring-2 ring-blue-200' : 'bg-slate-100 text-slate-600 ring-2 ring-slate-100'}`}>
                ...
                <div className="space-y-1 min-w-0">
                  ...
                  <span className="inline-block px-2 py-0.5 rounded-md bg-slate-100 text-slate-500 text-[9px] font-bold uppercase tracking-widest mt-1">{member.role}</span>
                </div>
              </div>
              {member.isSupervisor && (
  ...
  const renderIncentives = () => {
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-500">
        <div className="bg-slate-100 rounded-[2.5rem] p-8 border border-slate-200 shadow-sm">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
              <p className="text-[10px] font-black tracking-[0.3em] text-slate-400 uppercase">Yield</p>
              <h2 className="mt-1 text-xl font-black text-slate-900 tracking-tight uppercase">Marketing Yield</h2>
              <p className="mt-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {schedules.map(s => (
                <div key={s.q} className={`px-4 py-2.5 rounded-xl border flex flex-col items-center min-w-[90px] ${s.q === qInfo.q ? 'bg-blue-600 border-blue-600 text-white shadow-sm' : 'bg-white border-slate-200 text-slate-500'}`}>
                  <span className="text-[9px] font-black uppercase tracking-widest">{s.q}</span>
        ...
        <div className="bg-blue-50/80 border border-blue-100 rounded-[2rem] p-6 flex items-start gap-4">
          <div className="p-2 bg-blue-100 rounded-lg text-blue-600 shrink-0">
            <Info className="w-4 h-4" />
          </div>
          <div>
            <p className="text-[10px] font-black text-blue-900 uppercase tracking-widest mb-1">Policy note (under review)</p>
        ...
        <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">Personnel Yield Tracking</h4>
            <div className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-lg border border-slate-100">
              <Activity className="w-3.5 h-3.5 text-blue-500" />
              <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Real-Time Aggregation</span>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Marketing Personnel</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Audits ({qInfo.q})</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Quarterly Avg</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Tier Status</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Potential Payout</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {employeeIncentives.map((emp, idx) => (
                  <tr key={idx} className="group hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-5">
                      <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border ${emp.tierColor}`}>{emp.tier}</span>
                    </td>
                    <td className="px-6 py-5 text-right">
                      <p className={`text-sm font-black ${emp.avgScore >= 70 ? 'text-slate-900' : 'text-slate-300'}`}>{emp.potentialAmount}</p>
                      <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Est. {qInfo.payout}</p>
                    </td>
                  </tr>
                ))}
              </tbody>
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-500">
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-6">Marketing Department — Last 30 Days</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="p-6 bg-emerald-50 rounded-2xl border border-emerald-100">
              <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-1">Validated</p>
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-6">Export and detailed reports coming soon.</p>
        </div>
      </div>
    );
  };
      ...
      <div className="w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden pb-12">
          <div className="p-10 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
            <div className="flex items-center gap-6">
              <button onClick={() => setCurrentPage('queue')} className="flex items-center gap-2 p-3 text-slate-400 hover:text-slate-900 hover:bg-white rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2" aria-label="Back to Registry"><X className="w-5 h-5" /><span className="text-[10px] font-black uppercase tracking-widest">Back to Registry</span></button>
              <div>
                <h3 className="text-xl font-black text-slate-900 tracking-tight">
                  {isReadOnly ? (selectedItem.status === 'rejected' ? 'Rejected submission' : 'Archived submission') : 'Review submission'}
                </h3>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Record ID: {selectedItem.id}</p>
              </div>
            </div>
            {isReadOnly && (
              <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border ${selectedItem.status === 'rejected' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
                ...
                <span className="text-[9px] font-black uppercase tracking-widest">{selectedItem.status === 'rejected' ? 'Rejected' : 'Validated'}</span>
              </div>
            )}
            {!isReadOnly && (
              <div className={`px-4 py-2 rounded-xl border text-[9px] font-black uppercase tracking-widest flex items-center gap-2 ${calculatedScore.incentivePct >= 1 ? 'bg-emerald-50 text-emerald-600 border-emerald-100 animate-pulse' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>
          ...
          <div className="p-10 space-y-12">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              <div className="lg:col-span-7">
                <div className="bg-[#0b1222] rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden h-full">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-blue-600/20 blur-[60px] rounded-full" />
                  <div className="flex items-center justify-between mb-8 relative z-10">
                    <div className="flex items-center gap-4">
                      <ShieldCheck className="w-6 h-6 text-blue-400" />
                      <h4 className="text-sm font-black uppercase tracking-widest">Marketing Grading Matrix</h4>
                    </div>
                    {!isReadOnly && (
                      <button
                        onClick={() => setGrading(calculateInitialScores(selectedItem))}
                        className="px-3 py-1.5 bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all border border-blue-500/30 flex items-center gap-2"
                      >
                        ...
                        <div key={category} className="space-y-2">
                          <div className="flex justify-between items-center">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[9px] font-bold text-slate-300 uppercase tracking-widest">{category} ({(weight * 100).toFixed(0)}%)</span>
                            </div>
                  ...
                  <div className="mt-10 pt-10 border-t border-white/10 relative z-10">
                    <div className="flex items-baseline justify-between gap-4">
                      <p className="text-sm font-black text-slate-300 uppercase tracking-widest shrink-0">Final Weighted Score</p>
              ...
              <div className="lg:col-span-5 flex flex-col gap-6 min-h-0">
                {!isReadOnly ? (
                  <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm h-full flex flex-col min-h-0">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex-shrink-0">Supervisor reason</label>
                    <textarea
                      ref={justificationTextareaRef}
                      value={overrideReason}
                      onChange={e => setOverrideReason(e.target.value)}
                      placeholder="Required for Rejection or Score Override..."
                      className="w-full min-h-[140px] bg-slate-50 border border-slate-200 rounded-2xl p-5 text-sm font-medium text-slate-700 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 resize-none overflow-auto flex-1"
                    ...
                    <div className="grid grid-cols-2 gap-4 flex-shrink-0 pt-2">
                      <button onClick={() => handleAction('REJECT')} className="py-4 rounded-xl bg-red-50 text-red-600 text-[10px] font-black uppercase tracking-widest hover:bg-red-100 transition-colors">Recommend: Needs Changes</button>
                      <button onClick={() => handleAction('APPROVE')} className="py-4 rounded-xl bg-emerald-600 text-white text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-200">Recommend: Approve</button>
                    </div>
                  </div>
                ) : (
                  <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm space-y-6 h-full flex flex-col justify-center">
                    <div className="space-y-4 text-center">
                      ...
                      <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">Record reviewed</h4>
                      <p className="text-xs font-medium text-slate-500 max-w-[200px] mx-auto leading-relaxed">This submission has been processed and saved in the permanent records.</p>
                    </div>
                    <button onClick={() => setCurrentPage('queue')} className="w-full py-5 bg-slate-900 text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] transition-all mt-6">Back to Submissions</button>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3"><div className="w-8 h-8 bg-slate-900 rounded-lg flex items-center justify-center"><MessageSquare className="w-4 h-4 text-white" /></div><p className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Employee Narrative</p></div>
              <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3"><div className="w-8 h-8 bg-slate-900 rounded-lg flex items-center justify-center"><ClipboardCheck className="w-4 h-4 text-white" /></div><p className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Supervisor reason</p></div>
              <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                <p className="text-sm font-medium text-slate-600 leading-relaxed italic">&quot;{selectedItem.supervisorComment || (isReadOnly ? 'No supervisor reason saved.' : 'Waiting for admin review.')}&quot;</p>
              </div>
            </div>

            {/* Detailed Audit Review — dept. format aligned with Technical Review Terminal */}
            <div className="space-y-8">
              <div className="flex items-center gap-3"><div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center"><FileText className="w-4 h-4 text-white" /></div><p className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Detailed Audit Review</p></div>
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                {Object.keys(CATEGORY_ICONS).map((category) => {
                  ...
                  return (
                    <div key={category} className="bg-white border border-slate-100 rounded-[2.5rem] overflow-hidden shadow-sm flex flex-col">
                      <div className="p-8 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Category Score</p>
                      ...
                      <div className="p-8 space-y-4 flex-grow">
                        {panels.map((panel, taskIdx) => {
                          ...
                          return (
                            <div key={taskIdx} className="bg-slate-50 p-5 rounded-[1.5rem] border border-slate-100 flex flex-col justify-between gap-3 hover:border-blue-200 transition-colors">
                              <div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3"><div className="w-8 h-8 bg-slate-900 rounded-lg flex items-center justify-center"><Paperclip className="w-4 h-4 text-white" /></div><p className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Supporting documents</p></div>
              {selectedItem.attachments && selectedItem.attachments.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {selectedItem.attachments.map((file: any, idx: number) => (
                    <div key={idx} className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl group/file overflow-hidden">
                ...
                <p className="text-sm font-medium text-slate-400 italic py-4">No attached file</p>
              )}
            </div>
          </div>
        </div>
  ...
  return (
    <div className="w-full flex flex-col px-4 sm:px-6 md:px-8 lg:px-12 pb-6 md:pb-12">
      ...
      >
        ...
        <div className="hidden lg:hidden flex flex-wrap bg-white p-1.5 rounded-2xl border border-slate-100 shadow-sm w-fit ml-auto" role="navigation" aria-label="Supervisor navigation">
          {[
            { id: 'dashboard', label: 'Overview', icon: LayoutDashboard },
            ...
            return (
              <button
                ...
                className={`relative flex items-center gap-2.5 px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${currentPage === item.id ? 'z-10 bg-blue-900 text-white shadow-lg' : 'z-0 text-slate-400 hover:text-slate-600 hover:bg-slate-50'}`}
      ...
      <div className="hidden lg:block">
        <RoleSidenav
          ...
          items={[
            ...
            { id: 'queue', label: 'Submissions', description: countPendingReviewAudits(filteredPending) ? `${countPendingReviewAudits(filteredPending)} to review` : 'Review submissions', icon: ListTodo, badge: countPendingReviewAudits(filteredPending) },
            { id: 'team', label: 'Team', description: 'Your team', icon: Users },
            { id: 'incentives', label: 'Incentives', description: 'Incentives & tiers', icon: PesoCircleIcon },
          ]}
          activeId={currentPage}
          onSelect={(id) => {
            setCurrentPage(id as Page);
