    ...
    setTimeout(() => {
      ...
      if (matchedIdentities.length > 0) {
        ...
        if (!foundUser) {
          ...
          setFeedback({ 
            type: 'ERROR', 
            message: `This account does not have ${selectedRole} access.` 
          });
          setIsLoading(false);
          return;
        }
        ...
        if (foundUser.isActive === false) {
          ...
          setFeedback({ type: 'ERROR', message: 'This account has been deactivated.' });
          setIsLoading(false);
          return;
        }
        ...
        if (passkey !== foundUser.password) {
          ...
          setFeedback({ type: 'ERROR', message: 'Incorrect password. Please try again.' });
          setIsLoading(false);
          return;
        }
        ...
        setFeedback({ type: 'SUCCESS', message: `Logging in to ${foundUser.department}…` });
      } 
      else {
        onAddAuditEntry('AUTH_FAILURE', `Unrecognized identity login attempt: ${name.trim()}`, 'WARN');
        setFeedback({ type: 'ERROR', message: 'Username not found.' });
        setIsLoading(false);
        return;
      }
    ...
    <div className="w-full max-w-md animate-in fade-in zoom-in duration-700 relative">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <div className="w-full relative">
            ...
            <form onSubmit={handleSubmit} className="w-full space-y-5">
              ...
              >
                {isLoading ? <Activity className="w-5 h-5 animate-spin" /> : <>Log In <LogIn className="w-4 h-4" /></>}
              </button>
            </form>
          </div>
        </div>
      ...
      <div className="mt-8 flex flex-col items-center gap-4">
        <div className="flex items-center gap-2">
           ...
           <p className="text-slate-400 text-[9px] font-bold uppercase tracking-[0.3em]">AA2000 KPI System</p>
        </div>
      </div>
    </div>
  );
