...
const Navbar: React.FC<NavbarProps> = ({ user, onLogout, notifications, onDeleteNotification, validatedStats, registry, onUpdateRegistry }) => {
  ...
  const handlePasswordChange = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (userIndex === -1) {
        setPwdFeedback({ type: 'ERROR', msg: 'ACCESS DENIED: Identity not found' });
        setIsUpdatingPwd(false);
        return;
      }

      if (registry[userIndex].password !== pwdForm.current) {
        setPwdFeedback({ type: 'ERROR', msg: 'SECURITY BREACH: Invalid current passkey' });
        setIsUpdatingPwd(false);
        return;
      }

      if (pwdForm.new !== pwdForm.confirm) {
        setPwdFeedback({ type: 'ERROR', msg: 'MISMATCH: Passkeys do not align' });
        setIsUpdatingPwd(false);
        return;
      }

      if (pwdForm.new.length < 3) {
        setPwdFeedback({ type: 'ERROR', msg: 'REJECTED: Passkey complexity failure' });
        setIsUpdatingPwd(false);
        return;
      }
      ...
      setPwdFeedback({ type: 'SUCCESS', msg: 'SUCCESS: Security node updated' });
      setIsUpdatingPwd(false);
  ...
  return (
    <>
      {/* Security Settings Modal - Globally Centered */}
      {isSettingsOpen && (
          ...
          <div className="bg-white rounded-[3rem] w-full max-w-md p-10 md:p-12 shadow-2xl relative animate-in zoom-in-95 duration-500 overflow-hidden">
            ...
            <div className="space-y-8 relative">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">Security Node</h2>
                  <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Update Passkey Registry</p>
                </div>
              </div>
              ...
              <form onSubmit={handlePasswordChange} className="space-y-5">
                ...
                <div className="space-y-1.5 pt-2 border-t border-slate-100">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">New Passkey</label>
                  <div className="relative">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Confirm New Passkey</label>
                  <div className="relative">
      ...
      {/* AI Commentary Modal - Made wider to almost fit screen (max-w-6xl) */}
      {/* REMOVED */}
        ...
        <div className="max-w-[1800px] mx-auto w-full px-4 md:px-12 flex items-center justify-between">
          <div className="flex items-center gap-12">
            ...
            <div className="hidden lg:flex items-center gap-6">
              {/* AI Strategic Insight Button Removed */}
            </div>
          </div>
              ...
              {isNotifOpen && (
                <div className="absolute right-0 mt-4 w-80 bg-white rounded-[2rem] shadow-2xl border border-slate-100 py-6 z-[2000] animate-in slide-in-from-top-4 fade-in duration-300">
                  <div className="px-6 pb-4 border-b border-slate-50 flex items-center justify-between">
                    <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Broadcast Feed</h4>
                  ...
                  <div className="max-h-96 overflow-y-auto custom-scrollbar">
                    {userNotifications.length === 0 ? (
                      <div className="py-12 text-center text-slate-300 flex flex-col items-center gap-3">
                        ...
                        <p className="text-[9px] font-black uppercase tracking-widest">No Active Notifications</p>
                      </div>
                    ) : (
